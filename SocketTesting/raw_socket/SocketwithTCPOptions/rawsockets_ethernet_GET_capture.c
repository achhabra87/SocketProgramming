...
int main(int argc, char *argv[])
{
	...
	int dd = main_pcap(); // Recieving Packet INitialization by 
	...
	short pig_ack=0;
	int readin=0;
	printf ("Enter a source port number: ");
  	scanf ("%d",&readin);
	th_sport=(short)readin;
...
*/
	...
	*/
	...
	tcph->th_seq = htonl(ntohl(recv_tcph->th_ack));
	tcph->th_ack = htonl(ntohl(recv_tcph->th_seq));
