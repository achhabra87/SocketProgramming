...
char buffer[4096]; /* receve buffer */
...
int main(int argc, char **argv)
{
	...
	struct ip *recv_iph = (struct ip *) buffer;
	struct tcphdr *recv_tcph = (struct tcphdr *) (buffer + sizeof (struct ip));
	struct tcp_options *recv_tcpopt = (struct tcp_options *) (buffer + sizeof(struct ip) + sizeof(struct tcphdr));	
	...
	int u=0;
	do{
		u=recv(s, buffer, sizeof(buffer), 0);
		printbuffer(buffer);
	...
	*/
	...
	tcph->th_urp = 0;
	if (tcphdr_size % 4 != 0)
	{ /* takes care of padding to 32 bits */
